...
def randomly_select_appName(obj):
      # 按概率取其他用电器的数量
      num_others = np.random.choice([1,2,3,4],size=1,p=[0.4,0.35,0.15,0.10])
      other_apps = np.random.choice(obj.other_apps,size=num_others,replace=False,p=obj.p)
...
def data_generator(obj,data_set,p):
      ...
      - X : X_o-X_d-X_dd  shape = (samples,3)
      - y : y_o-y_d       shape = (samples,2)
      '''
      ...
      while True:
            ...
            assert len(y_o_seg)==len(y_d_seg)==batch_size
...
#            print('house_{}:'.format(house))
      ...
      if device_index == None:
            ...
            print('house_{}: appliance: {}   Index = {}'.format(house,appliance,device_index))
            X = data.mains
...
def segment(X,y,seg_length):
      ...
      if l==0: 
            l=-1  
      X_seg = [np.reshape(X[:,i][:-l],(-1,seg_length)) for i in range(X.shape[1])]
      y_seg = [np.reshape(y[:,i][:-l],(-1,seg_length)) for i in range(y.shape[1])]
